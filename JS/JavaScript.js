...
submit.addEventListener("click", FatchFilm);

function FatchFilm() {
    let url = `http://www.omdbapi.com/?s=${name.value}&type=${select.value}&apikey=31e6c428`
    ...
    fetch(url)
        ...
        .then(myJson => {
            ...
            for (let i = 0; i < obj.Search.length; i++) {
                buff += `<li><img src="${obj.Search[i].Poster}">${obj.Search[i].Title} (${obj.Search[i].Year})<button>Show info</button></li>`
            }
            ul.innerHTML = `${buff}`
...
function NextPage() {
    let url = `http://www.omdbapi.com/?s=${name.value}&type=${select.value}&page=${page + 1}&apikey=31e6c428`
    page += 1;
    let buff = ``;
